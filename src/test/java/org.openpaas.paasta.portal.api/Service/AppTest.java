...
public class AppTest extends Common {
...
//        ReflectionTestUtils.setField(common, "password", password);
    ...
    @Test
    public void getAppSummaryTest() throws IOException {

        when(appService.getAppSummary(GUID, TOKEN)).thenReturn(summaryApplicationResponse);
        summaryApplicationResponse = appService.getAppSummary(GUID, TOKEN);
        assertEquals(summaryApplicationResponse.getId(), GUID);
        assertEquals(summaryApplicationResponse.getName(), APPNAME);
    }
