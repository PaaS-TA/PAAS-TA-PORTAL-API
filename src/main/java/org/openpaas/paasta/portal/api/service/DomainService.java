...
public class DomainService extends Common {
    ...
    public PaginatedResponse getDomains(String status) throws Exception {
        ...
        switch (status) {
            case "all":
                return getAllDomains(connectionContext(), tokenProvider(adminUserName, adminPassword));
            case "private":
                return getPrivateDomains(connectionContext(), tokenProvider(adminUserName, adminPassword));
            case "shared":
                return getSharedDomains(connectionContext(), tokenProvider(adminUserName, adminPassword));
            default:
                throw new CloudFoundryException(HttpStatus.BAD_REQUEST, "Bad Request", "Invalid status");
        }
    }
    ...
    public ListDomainsResponse getOrgPrivateDomain(String token, String orguid) throws Exception {
        return cloudFoundryClient(this.connectionContext(), tokenProvider(token)).domains().list(ListDomainsRequest.builder().owningOrganizationId(orguid).build()).block();
    ...
    private ListDomainsResponse getAllDomains(final ConnectionContext context, final TokenProvider tokenProvider) {
        return cloudFoundryClient(context, tokenProvider).domains().list(ListDomainsRequest.builder().build()).block();
    }
    ...
    private ListDomainsResponse getAllDomains(final ConnectionContext context, final TokenProvider tokenProvider, final String names) {
        return cloudFoundryClient(context, tokenProvider).domains().list(ListDomainsRequest.builder().name(names).build()).block();
    }
    ...
    private ListPrivateDomainsResponse getPrivateDomains(final ConnectionContext context, final TokenProvider tokenProvider) {
        return cloudFoundryClient(context, tokenProvider).privateDomains().list(ListPrivateDomainsRequest.builder().build()).block();
    }
    ...
    private ListSharedDomainsResponse getSharedDomains(final ConnectionContext context, final TokenProvider tokenProvider) {
        return cloudFoundryClient(context, tokenProvider).sharedDomains().list(ListSharedDomainsRequest.builder().build()).block();
    }
    ...
    public Map addDomain(String token, final String domainName, final String orgId, boolean isShared) {
        ...
        try {
            boolean anyMatch = getAllDomains(connectionContext(), tokenProvider(token)).getResources().stream().anyMatch(domainResource -> domainName.equals(domainResource.getEntity().getName()));
            ...
            if (isShared) {
                final CreateSharedDomainResponse response = addSharedDomain(connectionContext(), tokenProvider(), domainName);
                LOGGER.debug("Response for adding shared domain is... {}", response);
                addedDomainName = response.getEntity().getName();
            } else {
                final CreatePrivateDomainResponse response = addPrivateDomain(connectionContext(), tokenProvider(token), domainName, orgId);
    ...
    private CreateSharedDomainResponse addSharedDomain(ConnectionContext context, TokenProvider tokenProvider, String domainName) {
        return cloudFoundryClient(context, tokenProvider).sharedDomains().create(CreateSharedDomainRequest.builder().name(domainName).build()).block();
    }

    public CreatePrivateDomainResponse addPrivateDomain(ConnectionContext context, TokenProvider tokenProvider, String domainName, String orgId) {
        return cloudFoundryClient(context, tokenProvider).privateDomains().create(CreatePrivateDomainRequest.builder().name(domainName).owningOrganizationId(orgId).build()).block();
    }
    ...
    public Map deleteDomain(String orgId, String domainName) throws Exception {
        ...
        try {
            ...
            List<DomainResource> domains = getAllDomains(connectionContext(), tokenProvider(adminUserName, adminPassword)).getResources().stream().filter(domain -> orgId.equals(domain.getEntity().getOwningOrganizationId())).filter(domain -> domainName.equals(domain.getEntity().getName())).collect(Collectors.toList());
            ...
            if (domains.size() > 0) {
                ...
                if (shared) {
                    ...
                    final DeletePrivateDomainResponse response = cloudFoundryClient(connectionContext(), tokenProvider(adminUserName, adminPassword)).privateDomains().delete(DeletePrivateDomainRequest.builder().privateDomainId(domain.getMetadata().getId()).build()).block();
