...
public class Common {
    ...
    public DefaultConnectionContext connectionContext() {
        if (paastaConnectionContext == null) {
            paastaConnectionContext = new PaastaConnectionContext(DefaultConnectionContext.builder().apiHost(apiTarget.replace("http://", "")).port(9022).skipSslValidation(skipSSLValidation).keepAlive(true).build(), new Date());
        } else {
